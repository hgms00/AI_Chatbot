...
from .inference_data.Color import Color
...
class Node:
    ...
    def solve(self):
        ...
        ret = fixed_ret if fixed_ret.__len__() != 0 else unfixed_ret
        if ret.__len__() != 0:
            if True in ret:
                state = True
            else:
        ...
        is_fixed = True if fixed_ret.__len__() != 0 else False
        ...
        if state is not None:
            if isinstance(self, NegativeNode) and need_reverse:
                state = not state if state != None else None
